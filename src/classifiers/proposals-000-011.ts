...
function parseMultisigScript(hex: string): { m: number; n: number; pubkeys: string[] } | null {
...
export function isOnSecp256k1Curve(pubkeyHex: string): boolean {
  const p = SECP256K1_P;
...
function containsAbortOpcode(hex: string): boolean {
  let pos = 0;
...
export function classifyGenesisBlock(ctx: ClassifierInput): '000' | null {
  if (ctx.block_number === 0 && ctx.is_coinbase) return '000';
  return null;
}

// Proposal 001 - Conflicting Coinbase Transaction IDs
export function classifyDuplicateCoinbase(ctx: ClassifierInput): '001' | null {
  if (ctx.block_number === 91722 || ctx.block_number === 91812) {
    if (ctx.tx_hash === 'e3bf3d07d4b0375638d5f1db5255fe07ba2c4cb067cd81b84ee974b6585fb468' ||
        ctx.tx_hash === 'd5d27987d2a3dfc724e359870c6644b40e497bdc0589a033220fe15429d88599') {
      return '001';
    }
  }
  return null;
...
export function classifyMtGoxScript(ctx: ClassifierInput): '003' | null {
  if (ctx.script_hex === '76a90088ac') return '003';
  return null;
}

// Proposal 004 - OP_RETURN
export function classifyOpReturn(ctx: ClassifierInput): '004' | null {
  if (ctx.script_hex.startsWith('6a')) return '004';
  return null;
}

// Proposal 011 - OP_VERIF / OP_VERNOTIF abort opcodes
export function classifyOpVerifAbort(ctx: ClassifierInput): '011' | null {
  if (containsAbortOpcode(ctx.script_hex)) return '011';
  return null;
}

// Proposal 005 - Off-Curve Public Key (P2PK)
export function classifyOffCurvePk(ctx: ClassifierInput): '005' | null {
...
export function classifyMultisigOffCurve(ctx: ClassifierInput): '006' | null {
  const parsed = parseMultisigScript(ctx.script_hex);
  if (parsed !== null) {
...
export function classifyTaprootOffCurve(ctx: ClassifierInput): '007' | null {
  const hex = ctx.script_hex;
...
export function classifySegwitWrongLength(ctx: ClassifierInput): '008' | null {
  const hex = ctx.script_hex;
...
export function classifyWrongHashPush(ctx: ClassifierInput): '009' | null {
  const hex = ctx.script_hex;
...
export function classifyInvalidPkEncoding(ctx: ClassifierInput): '010' | null {
  const hex = ctx.script_hex;
  if (hex.endsWith('ac')) {
    const pb = parseInt(hex.slice(0, 2), 16);
    if (pb >= 1 && pb <= 0x4b && hex.length === (pb + 2) * 2) {
      if (pb !== 33 && pb !== 65) {
        return '010'; // wrong-length key — can never be valid
      } else if (pb === 33 &&
                 hex.slice(2, 4) !== '02' && hex.slice(2, 4) !== '03') {
        return '010'; // 33-byte with invalid prefix
      }
